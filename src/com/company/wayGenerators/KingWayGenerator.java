...
public class KingWayGenerator extends WayGenerator{
    @Override
    public List<List<Coordinate>> getPossibleMoves(ChessGame game, Coordinate curr) {
        ...
        int row = curr.getRow();
        int col = curr.getCol();
        int[][] deltas = new int[][]{new int[]{1, 1}, new int[]{1, 0}, new int[]{1, -1},
                new int[]{0, 1}, new int[]{0, -1},
                new int[]{-1, 1}, new int[]{-1, 0}, new int[]{-1, -1}};
        for(int[] delta : deltas){
            row += delta[0];
            col += delta[1];
            if(row>=0 && col>=0 && row<chessField.length && col<chessField.length){
